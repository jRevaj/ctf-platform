# game-containers/base/entrypoint.sh
#!/bin/bash

# Debug info
echo "Starting entrypoint script" > /tmp/debug.log

# Set password for ctf-user for the challenge
echo "ctf-user:s3cret_t4rget2" | chpasswd
echo "Set password for ctf-user" >> /tmp/debug.log

# Create a hint file about target3
echo "Admin notes: The backup server at 172.1.0.4 uses key-based authentication only." > /home/ctf-user/admin_notes.txt
echo "The key is hidden somewhere in this server." >> /home/ctf-user/admin_notes.txt
chmod 644 /home/ctf-user/admin_notes.txt
chown ctf-user:ctf-user /home/ctf-user/admin_notes.txt

# Setup SSH
mkdir -p /home/ctf-user/.ssh
touch /home/ctf-user/.ssh/authorized_keys
echo "SSH directory created" >> /tmp/debug.log

# Set proper permissions
chown -R ctf-user:ctf-user /home/ctf-user/.ssh
chmod 700 /home/ctf-user/.ssh
chmod 600 /home/ctf-user/.ssh/authorized_keys
echo "SSH permissions set" >> /tmp/debug.log

# Setup authorized_keys with target1's public key for lateral movement
echo "ssh-rsa  root@baa0477b22bdbf47" >> /home/ctf-user/.ssh/authorized_keys

# Generate SSH host keys
ssh-keygen -A
echo "SSH host keys generated" >> /tmp/debug.log

# Create the hidden key for lateral movement to target3
mkdir -p /var/log/.backup

# Use a fixed key for target3
cat > /var/log/.backup/target3_key << 'EOL'
-----BEGIN OPENSSH PRIVATE KEY-----

-----END OPENSSH PRIVATE KEY-----
EOL

cat > /var/log/.backup/target3_key.pub << 'EOL'
ssh-rsa  pc1@Revaj
EOL

chmod 644 /var/log/.backup/target3_key
chown root:root /var/log/.backup/target3_key

# Set up log poisoning vulnerability
mkdir -p /var/log/nodeapp
touch /var/log/nodeapp/app.log
chmod 777 /var/log/nodeapp
chmod 666 /var/log/nodeapp/app.log

# Set up MongoDB
mkdir -p /data/db /var/log/mongodb
chmod 777 /data/db

# Start MongoDB in background - using direct command instead of service
echo "Starting MongoDB directly..." >> /tmp/debug.log
mongod --fork --logpath /var/log/mongodb.log --bind_ip 0.0.0.0
sleep 5
echo "MongoDB started" >> /tmp/debug.log

# Initialize MongoDB with setup script and add FLAG_PLACEHOLDER_6
echo "Initializing MongoDB and adding flag..." >> /tmp/debug.log

# Try different MongoDB shells
if command -v mongosh &> /dev/null; then
  echo "Using mongosh for initialization" >> /tmp/debug.log
  mongosh --file /target2/mongodb/setup.js || echo "Mongosh initialization error" >> /tmp/debug.log
elif command -v mongo &> /dev/null; then
  echo "Using mongo for initialization" >> /tmp/debug.log
  mongo /target2/mongodb/setup.js || echo "Mongo initialization error" >> /tmp/debug.log
else
  # Fallback to manual initialization if both mongosh and mongo are unavailable
  echo "MongoDB shells unavailable, using manual initialization" >> /tmp/debug.log
  mongosh --eval "
    db = db.getSiblingDB('ctf_app');
    db.secrets.drop();
    db.secrets.insertOne({
      title: 'Database Backup Key',
      content: 'FLAG_PLACEHOLDER_6',
      created_at: new Date()
    });
  " || echo "MongoDB manual initialization error" >> /tmp/debug.log
fi

# Set up and start the Node.js application
cd /target2/app
npm install || true
NODE_ENV=production node server.js &
echo "Node.js application started" >> /tmp/debug.log

# Start SSH with debug logging
echo "Starting SSH daemon" >> /tmp/debug.log
/usr/sbin/sshd -D -e

exec "$@"